...
class TestAStaleCloseIsRefusedAndSaysSo:
    ...
    def test_the_cycle_records_the_stale_refusal_on_the_funnel(self,
                                                              tmp_path):
        """Asserted end to end, because the reason is only useful if the
        caller actually passes `refused` and writes it down."""
        import inspect

        src = inspect.getsource(cycle.run_cycle)
        assert "refused=stale" in src, (
            "the cycle does not collect the reason, so a stale close "
            "would be indistinguishable from having no bars")
        assert "closed_market_close_too_stale" in src
